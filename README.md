...
One subtlety is directionality. The angle $\theta_{\min}(i,j) = \theta_{\min}(j,i)$ is symmetric, but in the confusion matrix 7→9 errors are much more frequent than 9→7. The angle alone cannot explain this. We therefore look at the closest paired directions between the two subspaces (the first left and right singular vectors of $U_{7,k}^\top U_{9,k}$) and measure how much variance each class has along its respective direction. Along these directions, class 7 images have about $1.4\times$ the variance of class 9 images. This is consistent with class 7’s greater spread toward class 9’s subspace and is one factor that may contribute to the directional asymmetry — though it does not fully account for the count difference on its own.
